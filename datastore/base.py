...
class BaseDatastore(abc.ABC):
    ...
    represents the spatial grid index of the data. The actual latitude, longitude coordinates
    of the grid points should be stored in the coordinates of the
    dataarray or dataset with the `grid_index` dimension as the coordinate for
    each of the `latitude` and `longitude` coordinates.
    """
    ...
    def land_sea_mask(self) -> xr.DataArray:
        """
        Return the land and sea mask for the dataset, with spatial dimensions
        stacked. Where the value is 1, the grid point is a sea point, and
        where the value is 0, the grid point is a land point.

        Returns
        -------
        xr.DataArray
            The land and sea mask for the dataset, with dimensions
            `('grid_index',)`.
    ...
    @abc.abstractmethod
    def get_latlon(self, category: str) -> np.ndarray:
        """
        Return the latitude, longitude coordinates of the dataset as a numpy arrays for a
        ...
        -------
        np.ndarray
            The latitude, longitude coordinates of the dataset with shape `[n_grid_points, 2]`.
        """

    @property
    @abc.abstractmethod
    def coords_projection(self) :
        """Return the projection object for the coordinates. -> ccrs.Projection
    ...
    @functools.lru_cache
    def get_latlon_extent(self, category: str) -> List[float]:
        """
        ...
        """
        lat_lon = self.get_latlon(category, stacked=True)
        extent = [
            lat_lon[:, 0].min(),
            lat_lon[:, 0].max(),
            lat_lon[:, 1].min(),
...
class BaseRegularGridDatastore(BaseDatastore):
    ...
    coordinate value is assumed to be associated with `latitude` and `longitude`-values that
    allow the processed data-arrays can be reshaped back into into 2D
    latlon-gridded arrays.
    ...
    - `grid_shape_state` (property): 2D shape of the grid for the state
      variables.
    - `get_latlon` (method): Return the latitude, longitude coordinates of the dataset, with the
      option to not stack the coordinates (so that they are returned as a 2D
      grid).

    The operation of going from (lat,lon)-indexed regular grid
    ...
    LATLON_COORDS = ["latitude", "longitude"]
    ...
    def grid_shape_state(self) -> CartesianGridShape:
        ...
        -------
        CartesianGridShape:
            The shape of the grid for the state variables, which has `latitude` and
    ...
    @abc.abstractmethod
    def get_latlon(self, category: str, stacked: bool = True) -> np.ndarray:
        ...
        category : str
            The category of the dataset (state/forcing/static).
        stacked : bool
            Whether to stack the latitude, longitude coordinates. The parameter `stacked` has
            ...
            to `True` to match the behaviour of `BaseDatastore.get_latlon()` which
            always returns the coordinates stacked.

        Returns
        -------
        np.ndarray
            The latitude, longitude coordinates of the dataset, returned differently based on
            the value of `stacked`: - `stacked==True`: shape `(n_grid_points,
            2)` where
                               n_grid_points=N_lat*N_lon.
            - `stacked==False`: shape `(N_lat, N_lon, 2)`
        """
        pass
    ...
    def unstack_grid_coords(
        ...
        """
        Unstack the spatial grid coordinates from `grid_index` into separate `latitude`
        and `longitude` dimensions to create a 2D grid. Only performs unstacking if the
        ...
        if not isinstance(da_or_ds.indexes.get("grid_index"), MultiIndex):
            da_or_ds = da_or_ds.set_index(grid_index=self.LATLON_COORDS)
        ...
        xy_dim_order = [d for d in dims if d in self.LATLON_COORDS]

        if xy_dim_order != self.LATLON_COORDS:
            da_or_ds_unstacked = da_or_ds_unstacked.transpose("latitude", "longitude")
    ...
    def stack_grid_coords(
        ...
        """
        Stack the spatial grid coordinates (latitude and longitude) into a single `grid_index`
        dimension. Only performs stacking if the data is currently unstacked
        (has latitude and longitude dimensions).
        ...
        da_or_ds_stacked = da_or_ds.stack(grid_index=self.LATLON_COORDS)
    ...
    def num_grid_points(self) -> int:
        ...
        """
        return self.grid_shape_state.latitude * self.grid_shape_state.longitude