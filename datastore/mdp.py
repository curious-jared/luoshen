...
class MDPDatastore(BaseRegularGridDatastore):
    ...
    def step_length(self) -> int:
        """The length of the time steps in days.
        ...
        return (da_dt.values[0] / (3600*24*10e+8)).astype(int)
    ...
    def get_dataarray(
        ...
        for coord in ["latitude", "longitude"]:
            if "units" not in da_category[coord].attrs:
                da_category[coord].attrs["units"] = "。"

        # set multi-index for grid-index
        da_category = da_category.set_index(grid_index=self.LATLON_COORDS)
    ...
    def get_standardization_dataarray(self, category: str) -> xr.Dataset:
        ...
        # Add standardized state diff stats, as the same result of standardized original
        # state diff stats
        if category == "state":
    ...
    def land_sea_mask(self) -> xr.DataArray:
        """
        Produce a 0/1 mask for the land and sea points of the dataset
    ...
    def grid_shape_state(self):
        ...
        da_lat, da_lon = ds_state.latitude, ds_state.longitude
        assert da_lat.ndim == da_lon.ndim == 1
        return CartesianGridShape(latitude=da_lat.size, longitude=da_lon.size)

    def get_latlon(self, category: str, stacked: bool) -> ndarray:
        ...
        category : str
            The category of the dataset (state/forcing/static).
        stacked : bool
            Whether to stack the latitude, longitude coordinates.
        ...
        da_xs = ds_category.latitude
        da_ys = ds_category.longitude

        assert da_xs.ndim == da_ys.ndim == 1

        da_x, da_y = xr.broadcast(da_xs, da_ys)
        da_latlon = xr.concat([da_x, da_y], dim="grid_coord")

        if stacked:
            da_latlon = da_latlon.stack(grid_index=self.LATLON_COORDS).transpose(
                "grid_index",
                "grid_coord",
            )
        else:
            ...
            da_latlon = da_latlon.transpose(*dims)

        return da_latlon.values