...
from utils import load_datastore
...
def mk_2d_graph(xy, lev, land_sea_mask):
    if lev == 1:
        index_0 = 1
    else:
        index_0 = 1 + 3 ** (lev - 1)

    lx_index = np.arange(index_0, xy.shape[0] - 1 ,3 ** lev)
    ly_index = np.arange(index_0, xy.shape[1] - 1 ,3 ** lev)
    ...
    # add diagonal edges
    g.add_edges_from(
        [((x, y), (x + 1, y + 1)) for y in range(len(ly) - 1) for x in range(len(lx) - 1)]
        + [
            ((x + 1, y), (x, y + 1))
    ...
    node_mask = land_sea_mask[np.ix_(lx_index,ly_index)]
    assert node_mask.shape == (len(lx), len(ly))
    # remove nodes that are not on sea
    nodes_to_remove = [(i, j) for i in range(len(lx)) for j in range(len(ly)) if not node_mask[i, j]]
...
def create_graph(
    ...
    """
    Create graph components from `latlon` grid coordinates and store in
    `graph_dir_path`.
    ...
    land_sea_mask : np.ndarray
        Create graph considering the land and sea mask (Nx, Ny, 1).
    n_max_levels : int
    ...
    for lev in range(1, mesh_levels + 1):
        g = mk_2d_graph(xy, lev, land_sea_mask)
        if create_plot:
...
def cli(input_args=None):
    ...
    parser.add_argument(
        "--datastore_kind",
        type=str,
        help="Datastore to use (default: mdp)", 
        default="mdp",)   
...
if __name__ == "__main__":
    ...
    b = np.arange(100, 125, 0.083, dtype=np.float32)
    mg = np.meshgrid(a, b, indexing="ij")  # Use 'ij' indexing for (Nx,Ny)
    latlon = np.stack((mg[0], mg[1]), axis=-1)
    print(latlon.shape)
    land_sea_mask = np.random.randint(0, 2, size=latlon.shape[:-1], dtype=bool)
