...
class BaseGraphModel(ARModel):
    ...
    def prepare_clamping_params(
        self, datastore: BaseDatastore, lower_lims:dict, upper_lims:dict
    ):
        """
    ...
    def get_clamped_new_state(self, state_delta, now_state):
        """
        Clamp prediction to valid range supplied
        Returns the clamped new state after adding delta to original state

        Instead of the new state being computed as
        $X_{t+1} = X_t + \\delta = X_t + model(\\{X_t,...\\}, forcing)$
        The clamped values will be
        $f(f^{-1}(X_t) + model(\\{X_t,... \\}, forcing))$
        ...
        now_state: (B, num_grid_nodes, feature_dim)
        """

        # Assign new state, but overwrite clamped values of each type later
        new_state = now_state + state_delta
        ...
        if self.clamp_lower_upper_idx.numel() > 0:
            ...
            new_state[:, :, idx] = self.clamp_lower_upper(
                self.inverse_clamp_lower_upper(now_state[:, :, idx])
                + state_delta[:, :, idx]
            )
        ...
        if self.clamp_lower_idx.numel() > 0:
            ...
            new_state[:, :, idx] = self.clamp_lower(
                self.inverse_clamp_lower(now_state[:, :, idx])
                + state_delta[:, :, idx]
            )
        ...
        if self.clamp_upper_idx.numel() > 0:
            ...
            new_state[:, :, idx] = self.clamp_upper(
                self.inverse_clamp_upper(now_state[:, :, idx])
                + state_delta[:, :, idx]
            )
    ...
    def predict_step(self, now_state, forcing):
        """
        Step state one step ahead using prediction model, X_t -> X_t+1
        now_state: (B, num_grid_nodes, feature_dim), X_t
        prev_state: (B, num_grid_nodes, feature_dim), X_{t-1}
        forcing: (B, num_grid_nodes, forcing_dim)
        """
        batch_size = now_state.shape[0]
        ...
        if self.args.clamping:
            ...
            new_state = self.get_clamped_new_state(rescaled_delta_mean, now_state)
        else:
            new_state = now_state + rescaled_delta_mean
