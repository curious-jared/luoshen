...
class WeatherDataset(torch.utils.data.Dataset):
    ...
    ar_steps : int, optional
        The number of autoregressive steps. Default is 3,like xt-1,xt forecast xt+1...t+3
    ...
    num_future_forcing_steps: int, optional
        ...
        t. Default is 1. 
        !!!! if add atmospheric forcing, pay attention to being different from time component forcing
    ...
    def __init__(
        ...
        # The current implementation requires at least 1 time step for the
        # initial state.
        self.init_steps = 1
    ...
    def __len__(self):
        if self.datastore.is_forecast:
            ...
            # and then take the first (1 + ar_steps) forecast times. In
            ...
            forecast_timewindow = max(self.init_steps, self.num_past_forcing_steps + 1) + self.ar_steps + (self.num_future_forcing_steps - 1)
            if n_forecast_steps < forecast_timewindow:
                raise ValueError(
                    "The number of forecast steps available "
                    f"({n_forecast_steps}) is less than the required for "
            ...
            #   - past forcing: max(1, self.num_past_forcing_steps) (at least 1
            ...
            return (
                ...
                - self.init_steps
                - self.ar_steps
                - max(0, self.num_past_forcing_steps - 1)
                - max(0, self.num_future_forcing_steps - 1)
                + 1
            )
    ...
    def _slice_state_time(self, da_state, idx, n_steps: int):
        ...
        available for the 1 initial states and any corresponding forcings
        ...
        if self.datastore.is_forecast:
            start_idx = max(0, self.num_past_forcing_steps + 1 - self.init_steps)
            end_idx = max(self.init_steps, self.num_past_forcing_steps + 1) + n_steps
            ...
            # sample per forecast, always starting at forecast time 1.
            ...
            # dataset.
            start_idx = idx + max(0, self.num_past_forcing_steps + 1 - self.init_steps)
            end_idx = (
                idx + max(self.init_steps, self.num_past_forcing_steps + 1) + n_steps
            )
    ...
    def _slice_forcing_time(self, da_forcing, idx, n_steps: int):
        ...
        if self.datastore.is_forecast:
            ...
            offset = max(self.init_steps, self.num_past_forcing_steps + 1)
            for step in range(n_steps):
                start_idx = offset + step - self.num_past_forcing_steps - 1
                end_idx = offset + step + self.num_future_forcing_steps - 1

                current_time = (
                    da_forcing.analysis_time[idx]
                    + da_forcing.elapsed_forecast_duration[offset + step - 1]
                )
            ...
            # the dataset.
            offset = idx + max(self.init_steps, self.num_past_forcing_steps + 1)
            for step in range(n_steps):
                start_idx = offset + step - self.num_past_forcing_steps - 1
                end_idx = offset + step + self.num_future_forcing_steps - 1
                ...
                # time coordinates
                current_time = da_forcing.time[offset + step - 1]
                da_sliced = da_sliced.expand_dims(
                    dim={"time": [current_time.values]}
                )
    ...
    def _build_item_dataarrays(self, idx):
        ...
        # init_state = 1
        da_init_states = da_state.isel(time=slice(0, 1))
        da_target_states = da_state.isel(time=slice(1, None))
    ...
    def __getitem__(self, idx):
        ...
        # init_states: (1, N_grid, d_features)
    ...
    def create_dataarray_from_tensor(
        ...
        for grid_coord in ["latitude", "longitude"]:
            if (
